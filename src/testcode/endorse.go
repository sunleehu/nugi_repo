...
func addOrgs(stub shim.ChaincodeStubInterface) pb.Response {
	...
	nargs = append(nargs, []byte(parameters[3]), epBytes, []byte(parameters[4]))
	fmt.Println("nargs", nargs)

	resp := invokeCC(stub, parameters[1], parameters[2], nargs)
	if err != nil {
		logger.Error(err)
	...
	err = stub.SetStateValidationParameter(parameters[0], resp.GetPayload())
...
func invokeCC(stub shim.ChaincodeStubInterface, channel, cc string, args [][]byte) pb.Response {
	ch := channel
	ccName := cc
