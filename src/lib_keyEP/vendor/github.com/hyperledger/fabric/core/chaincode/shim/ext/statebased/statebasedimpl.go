...
// Policy returns the endorsement policy as bytes
func (s *stateEP) PolicyOR() ([]byte, error) {
	spe := s.policyORFromMSPIDs()
	spBytes, err := proto.Marshal(spe)
	if err != nil {
		return nil, err
	}
...
// Policy returns the endorsement policy as bytes
func (s *stateEP) PolicyEx(master string) ([]byte, error) {
	spe, err := s.policyExFromMSPIDs(master)
	if err != nil {
		return nil, err
	}
	spBytes, err := proto.Marshal(spe)
...
func (s *stateEP) policyORFromMSPIDs() *cb.SignaturePolicyEnvelope {
	mspids := s.ListOrgs()
	sort.Strings(mspids)
	principals := make([]*mb.MSPPrincipal, len(mspids))
	sigspolicy := make([]*cb.SignaturePolicy, len(mspids))
...
func (s *stateEP) policyExFromMSPIDs(master string) (*cb.SignaturePolicyEnvelope, error) {
	if (len(s.orgs) < 2) {
		return nil, errors.New("Not enough organizations. At least two organizations are required.")
	}
	_, exists := s.orgs[master]
	if !exists {
		return nil, errors.New(fmt.Sprintf("No endorsement target organizations - %s", master))
	}
	mspids := make([]string, 0, len(s.orgs))
	mspids = append(mspids, master)
	for mspid := range s.orgs {
		if mspid != master {
	...
	// create the policy: it requires exactly 2 signature from all of the principals
	p := &cb.SignaturePolicyEnvelope{
		Version:    0,
		Rule:       cauthdsl.And(sigspolicy[0], cauthdsl.NOutOf(1, sigspolicy[1:])),
		Identities: principals,
	}
	return p, nil
}