...
func addOrgs(stub shim.ChaincodeStubInterface) pb.Response {
	...
	//err = ep.AddOrgs(statebased.RoleTypePeer, parameters[1:]...)		// EnableNodeOUs 사용해야 Role 구분 가능
	err = ep.AddOrgs(statebased.RoleTypeMember, parameters[1:]...)
	if err != nil {
		return shim.Error(err.Error())
	}
...
// policyEx
func policyEx(stub shim.ChaincodeStubInterface) pb.Response {
	_, parameters := stub.GetFunctionAndParameters()
	if len(parameters) < 2 {
		return shim.Error("No orgs to add specified")
	}
	...
	epBytes, err = ep.PolicyEx(parameters[1])
	if err != nil {
		return shim.Error(err.Error())
	}
